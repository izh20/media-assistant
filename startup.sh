...
start_web() {
    ...
    "$PROJECT_DIR/01-rag/.venv/bin/python" -c "
import subprocess, os, sys
with open('$LOG_DIR/web.log', 'w') as log:
    subprocess.Popen(
        [sys.executable, 'web_app.py'],
        cwd='$PROJECT_DIR/01-rag',
...
start_vision_llm() {
    local port="${1:-8080}"
    local model="Qwen2-VL-7B-Instruct-GGUF/Qwen2-VL-7B-Instruct-Q4_K_M.gguf"
    local mmproj="Qwen2-VL-7B-Instruct-GGUF/mmproj-Qwen2-VL-7B-Instruct-f16.gguf"

    echo "Starting Vision LLM service (port $port) with Qwen2-VL..."
    ...
    if [ ! -f "$MODEL_DIR/$mmproj" ]; then
        echo "  ERROR: mmproj file not found: $MODEL_DIR/$mmproj"
        return 1
    fi
    ...
    nohup llama-server \
        ...
        --host 0.0.0.0 \
        --port "$port" \
        > "$LOG_DIR/vision-$port.log" 2>&1 &

    echo "  Waiting for Vision LLM service..."
    for i in {1..60}; do
        curl -sf http://127.0.0.1:$port/v1/models >/dev/null 2>&1 && break
        sleep 2
    done
    ...
    if curl -sf http://127.0.0.1:$port/v1/models >/dev/null 2>&1; then
        echo "  Vision LLM service started on port $port"
    else
        echo "  ERROR: Vision LLM service failed. Check $LOG_DIR/vision-$port.log"
    fi
}
...
status() {
    ...
    echo ""
    echo "  Qwen2-VL models:"
    ls -lh "$MODEL_DIR/Qwen2-VL-7B-Instruct-GGUF"/*.gguf 2>/dev/null | head -5 || echo "  No Qwen2-VL GGUF files"
}
...
case "${1:-rag}" in
    ...
    llama)
        ...
        ;;
    vision)
        echo "=== Starting Vision LLM (Qwen2-VL) ==="
        PORT="${2:-8080}"
        start_vision_llm "$PORT"
        ;;
    media)
        echo "=== Starting Media Assistant ==="
        PORT="${2:-8080}"
        start_vision_llm "$PORT"
        start_media_web
        LOCAL_IP=$(get_local_ip)
        echo ""
        echo "Media Assistant ready!"
        echo "  Vision LLM: 0.0.0.0:$PORT"
        echo "  Web UI: http://$LOCAL_IP:8090"
        ;;
    stop)
        stop_services "${2:-}"
        ;;
    ...
    *)
        ...
        echo "  vision [port]   - Start Vision LLM (Qwen2-VL + mmproj)"
        echo "  media [port]    - Start Vision LLM + Media Assistant Web UI"
